...
def run_llm(cfg: DictConfig) -> None:
    ...
    elif cfg.condition == 'ALL_HERNIATION': 
        condition = 'herniation'
        data = '/work/robinpark/PID010A_clean/all_osclmric_reports.csv'
        definition = cfg.herniation_definition
        df_test = pd.read_csv(data, low_memory=False, index_col=0)
    ...
    with torch.no_grad():
        for i in range(len(li_prompts)):
            ...
            gen_ids = model.generate(input_ids=input_ids, max_new_tokens=1024, repetition_penalty=1.15,do_sample=False)[0]
    ...
    labeled_reports.to_csv(f'{cfg.llm_results_path}/april2024/summary-query/base-base/{cfg.model_name}_base_2step_{cfg.condition}{ivd_level}_new_template_yesno_scores_have_spec_summary_prompt.csv')
