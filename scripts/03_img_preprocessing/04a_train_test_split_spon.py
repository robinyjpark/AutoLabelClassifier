...
# IVD-level Labels
def import_labels():
    df_labels = pd.read_csv(f'{labels_path}/llama3_base_2step_ALL_SPON_new_template_yesno_scores_have_spec_summary_prompt.csv',index_col=0).reset_index(drop=True)
    return df_labels 

print('Importing and thresholding automated labels...')
df_labels = import_labels()
...
df_labels['results']=0
df_labels.loc[df_labels.yes_norm>0.9964028596878052, 'results'] = 1

# Merge with reports file to get identifiers 
df_all = pd.read_csv(all_reports_path)
df_labels = df_labels.merge(df_all, left_index=True, right_index=True, how='left')

# Create levels for each patient: L3-L4, L4-L5, L5-S1
df_labels['L3L4'] = df_labels['results']
df_labels['L4L5'] = df_labels['results']
df_labels['L5S1'] = df_labels['results']
df_labels = df_labels.drop(columns=['results'])

# Wide to long format
cols = ['report_no_hist', 'pred_conclusion', 'yes_score', 'no_score',
       'yes_norm', 'no_norm', 'pat_id', 'date', 'report']
df_labels = df_labels.melt(id_vars=cols, var_name='level', value_name='results')

df_labels['pat_id_date_level'] = df_labels['pat_id'] + '_' + df_labels['date'] + '_' + df_labels['level']
...
t2_s1_scans = []
for level in ['L3_L4', 'L4_L5', 'L5_S1']: 
...
list_pat_id_date_level = list(df_merged.pat_id_date_level)

# Get list of test subjects and manual labels 
df_test_subj1 = pd.read_csv(f'{test_reports_path}/manually_labeled_spon_test.csv')
df_test_subj2 = pd.read_csv(f'{test_reports_path}/manually_labeled_spon_train.csv')
df_test_subj_merged = pd.concat([df_test_subj1, df_test_subj2], axis=0).reset_index(drop=True)
df_test_subj = df_test_subj_merged[['pat_id', 'date']]

df_test_labels = df_test_subj_merged[['pat_id','date','global_label']].rename(columns={'global_label':'label'})

# Create column that merges pat ID and date
df_test_labels['pat_id_date'] = df_test_labels['pat_id'] + '_' + df_test_labels['date']

# Create levels for each patient: L3-L4, L4-L5, L5-S1
df_test_labels['L3L4'] = df_test_labels['label']
df_test_labels['L4L5'] = df_test_labels['label']
df_test_labels['L5S1'] = df_test_labels['label']
df_test_labels = df_test_labels.drop(columns=['label'])

# Wide to long format
df_test_labels = df_test_labels.melt(id_vars=['pat_id','date','pat_id_date'], var_name='level', value_name='label')

df_test_labels['pat_id_date_level'] = df_test_labels['pat_id_date'] + '_' + df_test_labels['level']
...
# save the splits
with open(f'{ivd_arrays_path}/april2024_splits/spon_train_val_test_split.pkl', 'wb') as handle:
    pickle.dump(osclmric_train_val_test_split, handle, protocol=pickle.HIGHEST_PROTOCOL)

# Import split data
with open(f'{ivd_arrays_path}/april2024_splits/spon_train_val_test_split.pkl', 'rb') as handle:
...
for pat_id_date_level in list_pat_id_date_level:
    if pat_id_date_level in ivd_dicts:
        if pat_id_date_level[:-5] in test_pat_date_list:
            ...
            test_pat_id_date.append(pat_id_date_level[:-5])
            ...
            val_pat_id_date.append(pat_id_date_level[:-5])
            ...
            train_pat_id_date.append(pat_id_date_level[:-5])
    else:
        continue
...
osclmric_array_dict['val_pat_id_date'] = val_pat_id_date
osclmric_array_dict['train_pat_id_date'] = train_pat_id_date

print('Saving arrays...')
# Pickle the dictionary
with open(f'{ivd_arrays_path}/april2024_splits/spon_arrays_dict.pkl', 'wb') as handle:
...
# Pickle the dictionary
with open(f'{ivd_arrays_path}/april2024_splits/spon_samples_dict.pkl', 'wb') as handle:
