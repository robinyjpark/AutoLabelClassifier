...
spnt_label_train_array = []
for i in range(len(label_train_array)):
    label = classify_ivd_v2_resnet(spnt.grading_model, ivd_train_array[i][np.newaxis,:], 'cuda:1')
    if label > 1:
        label = 1
    spnt_label_train_array.append(label)
...
osclmric_encodings['spnt_label_train_array'] = spnt_label_train_array
...
# Save the encodings
with open(f'{ivd_arrays_path}/spon_spinenet_encodings.pkl', 'wb') as handle:
