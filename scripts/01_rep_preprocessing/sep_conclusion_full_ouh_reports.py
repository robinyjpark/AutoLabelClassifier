...
def merge_report(row):
    ...
    list_sum = ['Conclusion', 'CONCLUSION', 'Impression', 'IMPRESSION',
                'SUMMARY', 'Summary']
    li_new = []
    # case 1: length of key is 1 & history
    # if len(row['seg_text'].keys()) == 1: 
    #     for key in row['seg_text']: 
    #         if key in list_hist: # replace the key with blank if history key is only key
    #             return row['report_text'].replace(key, '').replace(row['seg_text'][key], '')
    # case 2: summary does not exist
    if bool(set(row['seg_text'].keys()) & set(list_sum)) == False: 
        return '' # return nothing
    # case 3: multiple keys, summary exists
    else: 
        for key in row['seg_text']:
            if (key not in list_sum): # if not summary key 
                # if value (minus colon) not in headers, append
                if row['seg_text'][key][:-1] not in row['seg_text'].keys(): 
                    li_new.append(row['seg_text'][key])
            if key in list_sum: # if summary key, append with token
                li_new.append('\n[CONCLUSION]\n' + row['seg_text'][key])
...
def run_segmentation(cfg: DictConfig) -> None:
    ...
    seg_reports = seg_reports.loc[
        ...
        & (seg_reports['seg_length']<=5000) # ADD THIS BACK IF NEEDING A LIMIT
    ...
    seg_reports.to_csv(f'{cfg.clean_path}/segmented_unpaired_reports_sum_no_len_limit.csv')
